...
rosetta_functions <- rosetta_pkg$rosetta
rosetta <- rosetta_functions$rosetta
SoilData <- rosetta_functions$SoilData
```
...
process_data_with_rosetta <- function(df, vars, v=3, conf=NULL, include_sd=FALSE) {
  ...
  while (start_index <= length(df_valid_list_py)) {
    ...
    vg_params <- as.data.frame(matrix(mean, ncol = 5, byrow = TRUE))
    colnames(vg_params) <- c("theta_r", "theta_s", "alpha", "npar", "ksat")
    
    # Add model codes and version to the DataFrame
    vg_params[".rosetta.model"] <- factor(codes, levels = -1:5)
    vg_params[".rosetta.version"] <- v
  
    # Append results to the list
    results_list[[i]] <- vg_params
...
```{r}
# Load raster layers
db_raster <- rast("C:/R_Drive/Data_Files/LPKS_Data/R_Projects/VELMA.Soil/data/raw_data/db_0_Snohomish.tif")
sand_raster <- terra::crop(rast("C:/R_Drive/Data_Files/LPKS_Data/R_Projects/VELMA.Soil/data/raw_data/sand_0_Snohomish.tif"), terra::ext(db_raster))
silt_raster <- terra::crop(rast("C:/R_Drive/Data_Files/LPKS_Data/R_Projects/VELMA.Soil/data/raw_data/silt_0_Snohomish.tif"), terra::ext(db_raster))
clay_raster <- terra::crop(rast("C:/R_Drive/Data_Files/LPKS_Data/R_Projects/VELMA.Soil/data/raw_data/clay_0_Snohomish.tif"), terra::ext(db_raster))
...
```{r}
# Stack the rasters
soil_stack <- c(sand_raster, silt_raster, clay_raster)
...
normalized_soil_stack <- app(soil_stack, normalize_func)
